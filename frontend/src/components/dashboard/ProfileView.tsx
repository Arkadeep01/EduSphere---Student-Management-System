  ...
  return (
    <>
      ...
      <Card className="mb-4"><CardContent className="p-6 flex flex-col sm:flex-row items-start sm:items-center gap-4">
        <Avatar className="h-20 w-20"><AvatarFallback className="bg-gradient-brand text-white text-2xl">{user ? (user.first_name[0] + (user.last_name?.[0] || '')).toUpperCase() : '?'}</AvatarFallback></Avatar>
        <div><h2 className="text-2xl font-bold">{user ? `${user.first_name} ${user.last_name}` : ''}</h2><p className="text-muted-foreground">{user?.email}</p><p className="text-sm text-primary capitalize mt-1">{user?.role}</p></div>
      </CardContent></Card>
      <Tabs defaultValue="personal">
        ...
        <TabsContent value="personal"><Card><CardContent className="p-6 grid sm:grid-cols-2 gap-4 max-w-2xl">
          <div><Label>Full name</Label><Input defaultValue={user ? `${user.first_name} ${user.last_name}` : ''} /></div>
