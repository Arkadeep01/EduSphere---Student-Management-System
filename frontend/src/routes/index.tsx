...
import { GraduationCap, BookOpen, Users, ArrowRight, ChevronLeft, ChevronRight, Sparkles, Calendar, MapPin, Star, Quote, Trophy, Loader2 } from "lucide-react";
import { useEffect, useState } from "react";
import { usePublicStats, usePublicTeachers, usePublicEvents, usePublicGallery, usePublicFAQ } from "@/lib/usePublicData";
...
const topStudents = [
  { id: "1", name: "Riya Sen", class: "10-A", gpa: "4.0" },
  { id: "2", name: "Arnav Das", class: "10-B", gpa: "3.9" },
  { id: "3", name: "Priya Pal", class: "9-A", gpa: "3.8" },
];
    ...
    <div className="flex flex-col gap-5">
            ...
            <CardContent className="p-6 flex flex-col gap-4" style={{ minHeight: "200px" }}>
              ...
              <div className="flex items-center gap-3 pt-3 border-t">
                <Avatar className="h-10 w-10">
                  <AvatarFallback className="bg-gradient-brand text-primary-foreground text-xs font-semibold">{fb.initials}</AvatarFallback>
                </Avatar>
                <div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-2 items-center">
          {feedbacks.map((_, i) => (
            <button key={i} onClick={() => { if (i === idx) return; setDir(i > idx ? 1 : -1); setFlipping(true); setTimeout(() => { setIdx(i); setFlipping(false); }, 300); }}
              className="rounded-full transition-all duration-300"
              style={{ width: i === idx ? "24px" : "8px", height: "8px", background: i === idx ? "hsl(var(--primary))" : "hsl(var(--border))" }}
              aria-label={`Feedback ${i + 1}`} />
          ))}
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="icon" className="h-9 w-9 rounded-full" onClick={() => go(-1)} aria-label="Previous"><ChevronLeft className="h-4 w-4" /></Button>
          <Button size="icon" className="h-9 w-9 rounded-full" onClick={() => go(1)} aria-label="Next"><ChevronRight className="h-4 w-4" /></Button>
        </div>
      </div>
    </div>
  );
    ...
    <>
      ...
      <section className="relative h-137.5 overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto px-6 h-full flex items-center max-w-7xl">
          <div className="max-w-xl text-white">
            <FadeIn direction="up" delay={0}><p className="uppercase tracking-widest mb-2 text-primary font-semibold">EduSphere University</p></FadeIn>
            <FadeIn direction="up" delay={0.15}><h1 className="text-5xl md:text-7xl font-bold leading-tight">Together We'll Explore New Things</h1></FadeIn>
            <FadeIn direction="up" delay={0.3}><p className="mt-6 text-lg text-white/90">We believe everyone should have the opportunity to create progress through technology and education.</p></FadeIn>
            <FadeIn direction="up" delay={0.45}>
              <div className="flex gap-4">
                <Button className="mt-5" asChild><Link to={"/courses" as any}>Read Courses</Link></Button>
                <Button className="mt-5 text-muted-foreground" variant="outline" asChild><Link to={"/guidelines" as any}>Guidelines</Link></Button>
              </div>
            </FadeIn>
          </div>
        </div>
          ...
          <StaggerContainer className="grid md:grid-cols-3 bg-card text-card-foreground shadow-xl rounded-lg border overflow-hidden">
            <StaggerItem>
              <div className="p-8 border-r h-full hover:bg-muted/30 transition-colors">
                ...
                <h3 className="font-semibold text-lg">Admission Open</h3>
                <p className="text-muted-foreground mt-2 mb-4">Applications for 2026 are now open.</p>
                <Link to="/admissions" className="text-primary font-medium hover:underline inline-flex items-center gap-1">Learn More →</Link>
              </div>
            </StaggerItem>
            <StaggerItem>
              <div className="p-8 border-r h-full hover:bg-muted/30 transition-colors">
                <BookOpen className="h-10 w-10 text-primary mb-4" />
                <h3 className="font-semibold text-lg">Academic Programs</h3>
                <p className="text-muted-foreground mt-2 mb-4">Undergraduate and postgraduate programs.</p>
                <Link to="/courses" className="text-primary font-medium hover:underline inline-flex items-center gap-1">Learn More →</Link>
              </div>
            </StaggerItem>
            <StaggerItem>
              <div className="p-8 h-full hover:bg-muted/30 transition-colors">
                <Users className="h-10 w-10 text-primary mb-4" />
                <h3 className="font-semibold text-lg">Student Facilities</h3>
                <p className="text-muted-foreground mt-2 mb-4">Library, Laboratories and support.</p>
                <Link to={"/facilities" as any} className="text-primary font-medium hover:underline inline-flex items-center gap-1">Learn More →</Link>
              </div>
            </StaggerItem>
          </StaggerContainer>
        </div>
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
                ...
                <div className="flex flex-col sm:grid sm:grid-cols-12 gap-4 items-center">
                  <div className="sm:col-span-5 flex flex-col gap-6 w-full order-last sm:order-0">
                    <div className="relative group overflow-hidden rounded-3xl shadow-xl aspect-9/5 w-full sm:w-[155%] -ml-14 z-10">
                      <img src="/Colleagues.avif" alt="Students collaborating" className="w-full h-full object-cover transition-transform duration-500 ease-out group-hover:scale-105" />
                    ...
                    <div className="relative group overflow-hidden rounded-3xl shadow-xl aspect-2/3 w-[95%] sm:w-[120%] sm:-ml-8 z-10 sm:translate-y-4">
                      <img src="/group.avif" alt="University campus" className="w-full h-full object-cover transition-transform duration-500 ease-out group-hover:scale-105" />
                  ...
                  <div className="sm:col-span-7 relative">
                    ...
                    <div className="sm:col-span-7 relative group overflow-hidden rounded-2xl shadow-xl w-full h-70 sm:h-87.5 md:h-100">
                      <img src="/featuredStudent.avif" alt="Featured student" className="w-full h-full object-cover transition-transform duration-500 ease-out group-hover:scale-105" />
            ...
            <FadeIn direction="right">
              <div>
                <span className="text-primary font-semibold">About EduSphere</span>
                <h2 className="text-5xl font-bold mt-4">Admissions For Various Academic Disciplines</h2>
                <p className="mt-6 text-muted-foreground">We provide quality education through innovative teaching methods and modern facilities.</p>
                ...
                <Button className="mt-8" asChild><Link to={"/admissions" as any}>Read More</Link></Button>
              </div>
            </FadeIn>
          </div>
        </div>
              ...
              <StaggerContainer className="grid grid-cols-2 md:grid-cols-4">
                <StaggerItem>
                  <div className="flex items-center gap-3 p-6 border-r h-full">
                    ...
                    <div>
                      <p className="text-3xl font-bold text-primary"><Counter end={stats?.students || 0} />+</p>
                ...
                <StaggerItem>
                  <div className="flex items-center gap-3 p-6 border-r h-full">
                    ...
                    <div>
                      <p className="text-3xl font-bold text-primary"><Counter end={stats?.teachers || 0} />+</p>
                ...
                <StaggerItem>
                  <div className="flex items-center gap-3 p-6 border-r h-full">
                    ...
                    <div>
                      <p className="text-3xl font-bold text-primary"><Counter end={stats?.subjects || 0} />+</p>
                ...
                <StaggerItem>
                  <div className="flex items-center gap-3 p-6 h-full">
                    ...
                    <div>
                      <p className="text-3xl font-bold text-primary"><Counter end={stats?.classes || 0} />+</p>
      ...
      <section className="py-20">
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <StaggerContainer className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {teachers && teachers.length > 0 ? teachers.slice(0, 4).map((t: Record<string, unknown>) => (
                ...
                <Card className="hover-lift overflow-hidden h-full">
                  ...
                  <CardContent className="p-5 -mt-10">
                    <Avatar className="h-16 w-16 border-4 border-card">
                      <AvatarFallback className="bg-primary text-primary-foreground">{(t.name as string || "").split(" ").map(s => s[0]).slice(0, 2).join("")}</AvatarFallback>
                    </Avatar>
                    <h3 className="mt-3 font-semibold">{t.name as string}</h3>
                    <p className="text-sm text-muted-foreground">{t.subject as string || t.designation as string || ""}</p>
                    <div className="mt-3 flex items-center gap-1 text-sm">
                      <Star className="h-4 w-4 fill-warning text-warning" /> Featured
                    </div>
                  </CardContent>
                </Card>
              </StaggerItem>
            )) : <p className="text-muted-foreground col-span-full text-center py-10">No faculty data available yet.</p>}
          </StaggerContainer>
        </div>
      </section>
      ...
      <section className="py-20">
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <StaggerContainer className="grid md:grid-cols-3 gap-4">
            {events && events.length > 0 ? events.slice(0, 3).map((e: Record<string, unknown>) => (
              <StaggerItem key={e.id as string}>
                <Card className="hover-lift overflow-hidden h-full flex flex-col">
                  ...
                  <CardContent className="p-6 flex-1 flex flex-col justify-between">
                    <div>
                      <Badge variant="outline" className="mb-3">{e.type as string || (e.event_type as string) || ""}</Badge>
                    ...
                    <div className="mt-4 flex flex-col gap-1 text-sm text-muted-foreground pt-4 border-t">
                      <span className="flex items-center gap-2"><Calendar className="h-4 w-4" />{e.date ? new Date(e.date as string).toLocaleDateString("en-US", { month: "long", day: "numeric" }) : ""}</span>
                      <span className="flex items-center gap-2"><MapPin className="h-4 w-4" />{e.location as string || ""}</span>
                    </div>
                  </CardContent>
                </Card>
              </StaggerItem>
            )) : <p className="text-muted-foreground col-span-full text-center py-10">No upcoming events.</p>}
          </StaggerContainer>
        </div>
      </section>
              ...
              <StaggerItem key={f.t}>
                <Card className="hover-lift h-full">
                  <CardContent className="p-6">
                    <div className="h-12 w-12 rounded-lg bg-gradient-brand flex items-center justify-center mb-3"><Sparkles className="h-6 w-6 text-white" /></div>
      ...
      <section className="py-20 bg-muted/30">
        <div className="container mx-auto px-4">
          ...
          <StaggerContainer className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {galleryImages && galleryImages.length > 0 ? galleryImages.slice(0, 6).map(g => (
              <StaggerItem key={g.id}>
                <div className="aspect-4/3 rounded-xl relative overflow-hidden hover-lift cursor-pointer group">
                  <img src={g.image} alt={g.label} className="absolute inset-0 h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
            ...
            )) : <p className="text-muted-foreground col-span-full text-center py-10">No gallery images yet.</p>}
          </StaggerContainer>
        </div>
      </section>
      ...
      <section className="py-20">
        <div className="container mx-auto px-4 max-w-6xl">
          <FadeIn className="flex items-end justify-between mb-10">
            ...
            <Button asChild variant="ghost"><Link to={"/faq" as any}>View all <ArrowRight className="ml-1 h-4 w-4" /></Link></Button>
          </FadeIn>
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <FadeIn direction="left" delay={0.1}>
              <div>
                <Accordion type="single" collapsible className="space-y-2">
                  {faqs && faqs.length > 0 ? faqs.map((item: PublicFAQItem, i: number) => (
                    <AccordionItem key={item.id} value={`faq-${i}`} className="border rounded-lg px-4">
                    ...
                    [
                      ...
                      ["What is the fee structure?", "Annual fees range available depending on grade. Scholarships are available."],
                      ["Do you offer transport?", "Yes, with routes covering most neighborhoods in the metro area."],
                    ].map(([q, a], i) => (
                      <AccordionItem key={i} value={`faq-fallback-${i}`} className="border rounded-lg px-4">
                ...
                <div className="relative mt-8 rounded-2xl overflow-hidden h-52 bg-muted">
                  <div className="absolute inset-0 z-10 flex flex-col justify-center px-6 pointer-events-none">
                    ...
                    <p className="text-2xl font-bold leading-tight">Shaping futures,<br />one student at a time.</p>
                  </div>
                  <img src="/featuredStudent.avif" alt="EduSphere student" className="absolute bottom-0 right-0 h-full w-44 object-cover object-top" onError={e => { (e.target as HTMLImageElement).style.display = "none"; }} />
                  <div className="absolute bottom-4 left-6 z-10"><Button size="sm" asChild><Link to={"/faq" as any}>Read More</Link></Button></div>
                </div>
              </div>
            </FadeIn>
