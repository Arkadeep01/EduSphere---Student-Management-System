...
export const Route = createFileRoute("/teacher/timetable")({
  ...
  component: () => (
    <>
      <PageHeader title="Weekly Timetable" description="Your teaching schedule" />
              ...
              {timetable.map(d => {
                ...
                return (
                  <div key={d.day + time} className="p-3 rounded-lg bg-gradient-to-br from-primary/5 to-brand/5 border text-xs">
                    {slot ? <><p className="font-semibold">{slot[1]}</p><p className="text-muted-foreground">{slot[3]}</p></> : <span className="text-muted-foreground">—</span>}
                  </div>
                );
              })}
            </div>
