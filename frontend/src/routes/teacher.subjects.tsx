...
import { BookOpen, Upload, FileText, Video, Download, CheckCircle2, ChevronDown, ChevronRight, Layers, Users, CheckCircle, Plus, FileUp } from "lucide-react";
import { teacherSubjectData, classSubjectProgress } from "@/lib/mock-data";
...
function SubjectChaptersTab({
  ...
  classView?: string;
  onUploadSyllabus?: (chId: string) => void;
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {ch.syllabus ? (
                        <div className="flex items-center gap-2 flex-1">
                          <span className="text-xs text-muted-foreground">{ch.syllabus.fileName}</span>
                          <Button size="sm" variant="ghost" className="h-6 px-2 text-xs"><Download className="h-3 w-3 mr-1" />Download</Button>
                          <Button size="sm" variant="outline" className="h-6 px-2 text-xs" onClick={() => onUploadSyllabus?.(ch.id)}>Replace</Button>
                        </div>
                      ) : (
                        <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => onUploadSyllabus?.(ch.id)}>
...
export const Route = createFileRoute("/teacher/subjects")({
  ...
  component: () => {
    ...
    function handleUploadSyllabus(chId: string) {
      const name = `Syllabus_${chId}_${selectedClass}.pdf`;
      setClassProgress(prev => {
        const classData = prev[selectedClass];
        if (!classData) return prev;
        const updated = classData.map(ch =>
          ch.id === chId ? { ...ch, syllabus: { fileName: name, uploadedAt: new Date().toISOString().slice(0, 10) } } : ch
        );
        return { ...prev, [selectedClass]: updated };
      });
      toast.success(`Syllabus uploaded for ${selectedClass} — Chapter ${chId}`);
    }

    function handleUploadResource(chId: string) {
      const title = `${selectedClass} resource`;
      ...
      setClassProgress(prev => {
        ...
        if (!classData) return prev;
        const updated = classData.map(ch =>
          ch.id === chId ? { ...ch, resources: [...ch.resources, newRes] } : ch
        );
        return { ...prev, [selectedClass]: updated };
      });
      toast.success(`Resource added to ${selectedClass} — ${chId}`);
    }
